/****** Object:  Procedure [dbo].[UDSP_GetAlarmPerZoneForDrillDown]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [dbo].[UDSP_GetAlarmPerZoneForDrillDown] (  
...
-------------------------------------------------Alarms Total Number--------------------------------------------------------
	...
	if(@wherecode='')
		begin
			...
			--			'group by l.LocationID,l.Name,l.ParentLocationID,par.Name'
					...
					set @sqlstr='select count([AlarmID]) NoOfAlarms ,LocationID ,[LocationName] as LocationName ,ParentLocationID ,[ParentLocationName] as ParentLocationName '+
						'From [db_owner].[UDV_AlarmsLocation] where (touchpointlocationstateid = 1 or touchpointlocationstateid is null) AND LocationStateId = 1 '+
		...
		--				' group by l.LocationID,l.Name,l.ParentLocationID,par.Name'
						...
						'WHERE (touchpointlocationstateid = 1 or touchpointlocationstateid is null)  AND LocationStateId = 1 and '+@wherecode+
						' group by LocationID ,[LocationName]  ,ParentLocationID ,[ParentLocationName]   ' 
