/****** Object:  Procedure [dbo].[UDSP_Top10ChartGetAlarmPerZone]    Committed by VersionSQL https://www.versionsql.com ******/

Create PROCEDURE [dbo].[UDSP_Top10ChartGetAlarmPerZone](        @City nvarchar(100)=null		,
...
-------------------------------------------------Alarms Total Number--------------------------------------------------------
	...
	if(@wherecode='')
		begin
			set @sqlstr='select top(10) count([AlarmID]) as value,ZoneTitle1 as Name,21 as StoredProcedureID '+
						 'from [dbo].[UDV_AllAlarms]   '+
						 'group by  ZoneTitle1 order by 1 desc'
		end
	else
		begin
