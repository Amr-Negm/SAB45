/****** Object:  Procedure [dbo].[UDSP_CardUnHandledAlarmsLastWeek]    Committed by VersionSQL https://www.versionsql.com ******/

create PROCEDURE [dbo].[UDSP_CardUnHandledAlarmsLastWeek]
AS
BEGIN
...
8			LastYear
*/
EXEC	[dbo].[UDSP_CardUnHandledAlarms] @LastWeek,@EndDate,@SPValue1 output;
select @ActionDate=sysdatetime()
delete from [dbo].HBB_Analytics_Cards where [StoredProcedureID]=@UDSP_CardUnHandledAlarms and PeriodId=6;
insert into [dbo].HBB_Analytics_Cards([StoredProcedureID],[StoredProcedureValue],[ActionDate],PeriodId)
values 
(@UDSP_CardUnHandledAlarms,@SPValue1,@ActionDate,6);
end