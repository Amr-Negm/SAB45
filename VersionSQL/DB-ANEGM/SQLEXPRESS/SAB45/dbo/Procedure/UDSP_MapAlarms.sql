/****** Object:  Procedure [dbo].[UDSP_MapAlarms]    Committed by VersionSQL https://www.versionsql.com ******/

		CREATE PROCEDURE [dbo].[UDSP_MapAlarms](        @City nvarchar(100)=null		,
...
-------------------------------------------------Alarms Total Number--------------------------------------------------------
	...
	if(@wherecode='')
		begin
			set @sqlstr+='select  Longitude, Latitude,   [StationTitle1],count(*) as ''AlarmID'' from [dbo].[UDV_MapAlarmView] group by [Longitude],[Latitude],[StationTitle1]'
		...
		set @wherecode=SUBSTRING ( @wherecode ,0 ,LEN ( @wherecode ) -3  )
			set @sqlstr+='select  Longitude, Latitude,   [StationTitle1],count(*) as ''AlarmID'' from [dbo].[UDV_MapAlarmView] Where '+@wherecode+' group by [Longitude],[Latitude],[StationTitle1]'
		end
	--print @sqlstr
	execute(@sqlstr)
SET NOCOUNT OFF;
