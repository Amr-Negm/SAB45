/****** Object:  Procedure [dbo].[UDSP_ScenariosPerLocation1]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [dbo].[UDSP_ScenariosPerLocation1](        @City nvarchar(100)=null		,
...
-------------------------------------------------Alarms Total Number--------------------------------------------------------
	...
	if(@wherecode='')
		begin
			set @sqlstr+='select CityTitle1,PriorityTitle1 as "priority",count(*) as ptotal from [dbo].[T_AllAlarms] group by CityTitle1,PriorityTitle1 order by 1'
		end
	else
		begin
		set @wherecode=SUBSTRING ( @wherecode ,0 ,LEN ( @wherecode ) -3  )
		set @sqlstr+='select CityTitle1,PriorityTitle1 as "priority",count(*) as ptotal from [dbo].[T_AllAlarms] where '+@wherecode+' group by CityTitle1,PriorityTitle1 order by 1'
		end
	--print @sqlstr
	execute(@sqlstr)
SET NOCOUNT OFF;
