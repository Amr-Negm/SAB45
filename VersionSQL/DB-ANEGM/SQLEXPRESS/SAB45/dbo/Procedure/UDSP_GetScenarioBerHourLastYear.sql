/****** Object:  Procedure [dbo].[UDSP_GetScenarioBerHourLastYear]    Committed by VersionSQL https://www.versionsql.com ******/

create PROCEDURE [dbo].[UDSP_GetScenarioBerHourLastYear]
AS
BEGIN
...
8			LastYear
*/
delete from [dbo].HBB_UDTB_GetScenarioBerHour where [StoredProcedureID]=@UDSP_GetScenarioBerHour and PeriodId=8;
...
exec [dbo].[UDSP_GetScenarioBerHour]  @LastYear,@EndDate,NULL;
insert into [dbo].HBB_UDTB_GetScenarioBerHour(ScenarioTitle1 ,date ,total ,StoredProcedureID,ActionDate,Periodid)
SELECT ScenarioTitle1 ,date ,total ,@UDSP_GetScenarioBerHour,@EndDate,8 FROM #TMPGetScenarioBerHour;
