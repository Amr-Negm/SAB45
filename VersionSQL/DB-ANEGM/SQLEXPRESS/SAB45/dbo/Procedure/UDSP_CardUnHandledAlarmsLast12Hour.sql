/****** Object:  Procedure [dbo].[UDSP_CardUnHandledAlarmsLast12Hour]    Committed by VersionSQL https://www.versionsql.com ******/

create PROCEDURE [dbo].[UDSP_CardUnHandledAlarmsLast12Hour]
AS
BEGIN
...
8			LastYear
*/
EXEC	[dbo].[UDSP_CardUnHandledAlarms] @Last12Hour,@EndDate,@SPValue1 output;
select @ActionDate=sysdatetime()
delete from [dbo].HBB_Analytics_Cards where [StoredProcedureID]=@UDSP_CardUnHandledAlarms and PeriodId=3;
insert into [dbo].HBB_Analytics_Cards([StoredProcedureID],[StoredProcedureValue],[ActionDate],PeriodId)
values 
(@UDSP_CardUnHandledAlarms,@SPValue1,@ActionDate,3);
end