/****** Object:  Procedure [dbo].[UDSP_InvestigateAlarms_LessData_DelMe2]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE procedure  [dbo].[UDSP_InvestigateAlarms_LessData_DelMe2] 
...
FROM		T_Alarms				A
INNER JOIN	LT_AlarmStatuses		AStatus	ON A.AlarmStatusID = AStatus.AlarmStatusID
INNER JOIN	LT_AlarmCategories		ACat	ON A.AlarmStatusID = ACat.AlarmCategoryID
INNER JOIN	LT_AlarmClassifications	AClass	ON A.AlarmStatusID = AClass.AlarmClassificationID
...
SELECT @Sql =
CONCAT(
'SELECT * FROM InvestigatAlarmsTable_Temp order by ',@SortBy,' ',@SortType,' OFFSET ',@PageNo,'*',@PageSize,' ROWS 
