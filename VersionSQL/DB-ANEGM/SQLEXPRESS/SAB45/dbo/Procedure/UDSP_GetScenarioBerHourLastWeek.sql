/****** Object:  Procedure [dbo].[UDSP_GetScenarioBerHourLastWeek]    Committed by VersionSQL https://www.versionsql.com ******/

create PROCEDURE [dbo].[UDSP_GetScenarioBerHourLastWeek]
AS
BEGIN
...
8			LastYear
*/
delete from [dbo].HBB_UDTB_GetScenarioBerHour where [StoredProcedureID]=@UDSP_GetScenarioBerHour and PeriodId=6;
...
exec [dbo].[UDSP_GetScenarioBerHour]  @LastWeek,@EndDate,NULL;
insert into [dbo].HBB_UDTB_GetScenarioBerHour(ScenarioTitle1 ,date ,total ,StoredProcedureID,ActionDate,Periodid)
SELECT ScenarioTitle1 ,date ,total ,@UDSP_GetScenarioBerHour,@EndDate,6 FROM #TMPGetScenarioBerHour;
