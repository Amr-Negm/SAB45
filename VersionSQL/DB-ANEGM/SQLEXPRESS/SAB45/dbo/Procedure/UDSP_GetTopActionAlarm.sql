/****** Object:  Procedure [dbo].[UDSP_GetTopActionAlarm]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [dbo].[UDSP_GetTopActionAlarm](        @City nvarchar(100)=null		,
...
-------------------------------------------------Alarms Total Number--------------------------------------------------------
	...
	if(@wherecode='')
		begin
			set @sqlstr='select ActionTypeTitle1,count(*) as value from [dbo].[UDV_AllAlarmsActions] group by ActionTypeTitle1 order by 2 desc'
		end
	else
		begin
		set @wherecode=SUBSTRING ( @wherecode ,0 ,LEN ( @wherecode ) -3  )
		set @sqlstr='select ActionTypeTitle1,count(*) as value from [dbo].[UDV_AllAlarmsActions] Where '+@wherecode +'group by ActionTypeTitle1 order by 2 desc'
		end
	print @sqlstr
	execute(@sqlstr)
