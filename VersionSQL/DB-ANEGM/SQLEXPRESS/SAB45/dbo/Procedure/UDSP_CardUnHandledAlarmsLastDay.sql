/****** Object:  Procedure [dbo].[UDSP_CardUnHandledAlarmsLastDay]    Committed by VersionSQL https://www.versionsql.com ******/

create PROCEDURE [dbo].[UDSP_CardUnHandledAlarmsLastDay]
AS
BEGIN
...
8			LastYear
*/
EXEC	[dbo].[UDSP_CardUnHandledAlarms] @LastDay,@EndDate,@SPValue1 output;
select @ActionDate=sysdatetime()
delete from [dbo].HBB_Analytics_Cards where [StoredProcedureID]=@UDSP_CardUnHandledAlarms and PeriodId=4;
insert into [dbo].HBB_Analytics_Cards([StoredProcedureID],[StoredProcedureValue],[ActionDate],PeriodId)
values 
(@UDSP_CardUnHandledAlarms,@SPValue1,@ActionDate,4);
end