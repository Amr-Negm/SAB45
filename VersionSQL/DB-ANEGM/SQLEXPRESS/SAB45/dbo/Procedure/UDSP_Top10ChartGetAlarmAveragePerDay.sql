/****** Object:  Procedure [dbo].[UDSP_Top10ChartGetAlarmAveragePerDay]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [dbo].[UDSP_Top10ChartGetAlarmAveragePerDay](
...
if(@wherecode='')
	begin
		set @sqlstr='				  select count([AlarmScenarioID]) as ''AvgBerDay'', cast(DATENAME(weekday,SelectedCreationDate) as varchar(3)) as Dayname,19 as StoredProcedureID '+
						'from [dbo].[UDV_AllScenarios] where IsSelected=1 and SelectedCreationDate is not null '+				
	...
	set @wherecode=SUBSTRING ( @wherecode ,0 ,LEN ( @wherecode ) -3  )
		set @sqlstr='				  select count([AlarmScenarioID]) as ''AvgBerDay'', cast(DATENAME(weekday,SelectedCreationDate) as varchar(3)) as Dayname,19 as StoredProcedureID '+
						'from [dbo].[UDV_AllScenarios] where IsSelected=1 and SelectedCreationDate is not null AND '+@wherecode+				
						 ' group by cast(DATENAME(weekday,SelectedCreationDate) as varchar(3))'
	end
