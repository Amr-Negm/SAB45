/****** Object:  Procedure [dbo].[UDSP_AlarmsPerCameraLastDay]    Committed by VersionSQL https://www.versionsql.com ******/

create PROCEDURE [dbo].[UDSP_AlarmsPerCameraLastDay]
AS
BEGIN
...
Delete from [dbo].HBB_UDTB_AlarmsPerCamera where [StoredProcedureID]=@UDSP_AlarmsPerCamera and PeriodId=4;

CREATE TABLE #TMPAlarmsPerCamera (SourceName nvarchar(150),No_OF_Alarm nvarchar(150));

INSERT INTO #TMPAlarmsPerCamera
exec [dbo].[UDSP_AlarmsPerCamera] @LastDay,@EndDate

insert into [dbo].HBB_UDTB_AlarmsPerCamera(SourceName ,No_OF_Alarm ,StoredProcedureID,[ActionDate],PeriodId)
SELECT SourceName ,No_OF_Alarm ,@UDSP_AlarmsPerCamera,@EndDate,4 FROM #TMPAlarmsPerCamera;
