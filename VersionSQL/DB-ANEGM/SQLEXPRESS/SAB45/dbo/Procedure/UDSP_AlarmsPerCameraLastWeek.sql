/****** Object:  Procedure [dbo].[UDSP_AlarmsPerCameraLastWeek]    Committed by VersionSQL https://www.versionsql.com ******/

create PROCEDURE [dbo].[UDSP_AlarmsPerCameraLastWeek]
AS
BEGIN
...
Delete from [dbo].HBB_UDTB_AlarmsPerCamera where [StoredProcedureID]=@UDSP_AlarmsPerCamera and PeriodId=6;

CREATE TABLE #TMPAlarmsPerCamera (SourceName nvarchar(150),No_OF_Alarm nvarchar(150));

INSERT INTO #TMPAlarmsPerCamera
exec [dbo].[UDSP_AlarmsPerCamera] @LastWeek,@EndDate

insert into [dbo].HBB_UDTB_AlarmsPerCamera(SourceName ,No_OF_Alarm ,StoredProcedureID,[ActionDate],PeriodId)
SELECT SourceName ,No_OF_Alarm ,@UDSP_AlarmsPerCamera,@EndDate,6 FROM #TMPAlarmsPerCamera;
