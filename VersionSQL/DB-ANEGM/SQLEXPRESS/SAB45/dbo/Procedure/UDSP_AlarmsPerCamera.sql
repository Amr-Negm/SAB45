/****** Object:  Procedure [dbo].[UDSP_AlarmsPerCamera]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [dbo].[UDSP_AlarmsPerCamera](
...
set @sqlstr+=' select  TouchPointCode as ''SourceName'',count(*) as ''No_OF_Alarm''  from [dbo].[UDV_AllAlarmsNew]  group by  TouchPointCode order by 1 desc'
...
set @sqlstr+='select  TouchPointCode as ''SourceName'',count(*) as ''No_OF_Alarm''  from [dbo].[UDV_AllAlarmsNew] where '+@wherecode+'  group by  TouchPointCode order by 1 desc'
end
