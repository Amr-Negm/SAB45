/****** Object:  Procedure [dbo].[UDSP_GetAlarmPerStatusRealTime]    Committed by VersionSQL https://www.versionsql.com ******/

  
CREATE PROCEDURE [dbo].[UDSP_GetAlarmPerStatusRealTime]( @City nvarchar(max)=  null,  
...
-------------------------------------------------Alarms Total Number--------------------------------------------------------  
  set @wherecode=''  
    ...
    begin  
	 ...
	 set @wherecode+='  CONVERT(VARCHAR(10), [Occurance_Date],110) <='''+ CONVERT(VARCHAR(10), @EndDate   , 110)+''' AND'  
    end  
  
    if(@wherecode='')  
    begin  
     set @sqlstr='select AlarmStatusTitle1,count(*) as value from [dbo].[UDV_AllAlarms] group by AlarmStatusTitle1'  
    end  
    else  
    begin  
    set @wherecode=SUBSTRING ( @wherecode ,0 ,LEN ( @wherecode ) -3  )  
     set @sqlstr='select AlarmStatusTitle1,count(*) as value from [dbo].[UDV_AllAlarms] Where '+@wherecode+' group by AlarmStatusTitle1'  
    end  
	--print @sqlstr
  execute(@sqlstr)  
END