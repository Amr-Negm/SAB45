/****** Object:  Procedure [dbo].[UDSP_Top10ChartGetActiveAlarm]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [dbo].[UDSP_Top10ChartGetActiveAlarm](        @City nvarchar(100)=null		,
...
-------------------------------------------------Alarms Total Number--------------------------------------------------------
	...
	if(@wherecode='')
		begin
			set @sqlstr='select top(10) count([AlarmID]) as value,AlarmDefinitionTitle1 as Name,18  as StoredProcedureID '+
						 'from [dbo].[UDV_AllAlarms]  where Response_Date is null '+
						 'group by  AlarmDefinitionTitle1 order by 1 desc'
		end
	else
		begin
		set @wherecode=SUBSTRING ( @wherecode ,0 ,LEN ( @wherecode ) -3  )
		set @sqlstr='select top(10) count([AlarmID]) as value,AlarmDefinitionTitle1 as Name18  as StoredProcedureID '+
						 'from [dbo].[UDV_AllAlarms]  where Response_Date is null AND '+@wherecode+
						 ' group by  AlarmDefinitionTitle1 order by 1 desc'
		end
	--print @sqlstr
	execute(@sqlstr)
