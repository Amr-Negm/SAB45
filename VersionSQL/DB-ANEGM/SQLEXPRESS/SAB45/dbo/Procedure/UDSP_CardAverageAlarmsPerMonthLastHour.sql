/****** Object:  Procedure [dbo].[UDSP_CardAverageAlarmsPerMonthLastHour]    Committed by VersionSQL https://www.versionsql.com ******/

create PROCEDURE [dbo].[UDSP_CardAverageAlarmsPerMonthLastHour]
AS
BEGIN
...
EXEC	[UDSP_CardAverageAlarmsPerMonth]  @LastHour,@EndDate,@SPValue1 output;

select @ActionDate=sysdatetime()
delete from [dbo].HBB_Analytics_Cards where [StoredProcedureID]=@UDSP_CardAverageAlarmsPerMonth and PeriodId=1;
insert into [dbo].HBB_Analytics_Cards([StoredProcedureID],[StoredProcedureValue],[ActionDate],PeriodId)
values 
(@UDSP_CardAverageAlarmsPerMonth,@SPValue1,@ActionDate,1);
end