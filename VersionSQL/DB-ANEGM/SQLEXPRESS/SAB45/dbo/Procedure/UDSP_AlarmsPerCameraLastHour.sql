/****** Object:  Procedure [dbo].[UDSP_AlarmsPerCameraLastHour]    Committed by VersionSQL https://www.versionsql.com ******/

Create PROCEDURE [dbo].[UDSP_AlarmsPerCameraLastHour]
AS
BEGIN
...
Delete from [dbo].HBB_UDTB_AlarmsPerCamera where [StoredProcedureID]=@UDSP_AlarmsPerCamera and PeriodId=1;

CREATE TABLE #TMPAlarmsPerCamera (SourceName nvarchar(150),No_OF_Alarm nvarchar(150));

INSERT INTO #TMPAlarmsPerCamera
exec [dbo].[UDSP_AlarmsPerCamera] @LastHour,@EndDate

insert into [dbo].HBB_UDTB_AlarmsPerCamera(SourceName ,No_OF_Alarm ,StoredProcedureID,[ActionDate],PeriodId)
SELECT SourceName ,No_OF_Alarm ,@UDSP_AlarmsPerCamera,@EndDate,1 FROM #TMPAlarmsPerCamera;
