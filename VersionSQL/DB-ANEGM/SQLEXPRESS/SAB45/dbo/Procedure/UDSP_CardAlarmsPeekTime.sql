/****** Object:  Procedure [dbo].[UDSP_CardAlarmsPeekTime]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [dbo].[UDSP_CardAlarmsPeekTime]      @City nvarchar(100)=null	,
		 ...
		 select @x=a.hour from( 
							...
							from [dbo].[UDV_AllAlarms] 
											 where  CityTitle1=isnull(@City,CityTitle1)						AND
													...
													order by 1 desc) a
select @x=isnull(@x,'00')
select @PeekTime=(select cast (dbo.MinutesToDuration_TwoOctet(@x) as varchar(20)) as 'PeekTime')
select @PeekTime
SET NOCOUNT OFF;
END